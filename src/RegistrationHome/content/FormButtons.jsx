...
export default class FormButtons extends React.Component {
  ...
  buttonArrayRender() {
    ...
    return buttonOptions.map((buttonOption, index) => {
      ...
      return (
        <div className={classStr} key={index + buttonOption + 'parent'}>
          <button type="button" className="form-button__button btn bg-citizen_labs_orange txt-white montserrat"
                  key={index + buttonOption}
                  onClick={clickAction}><h3 key={index + buttonOption + 'H3'}>{buttonOption}</h3></button>
        </div>);
    });
  }
